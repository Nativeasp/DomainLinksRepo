...
def generate_policy_v2():
    ...
    if controls:
        ...
        hdr[0].text = 'Control Code'
        hdr[1].text = 'Display Name'
        hdr[2].text = 'Description'
        hdr[3].text = 'Objective'
        for c in controls:
            row = ctrl_table.add_row().cells
            row[0].text = str(c.get('ControlCode', ''))
            row[1].text = str(c.get('DisplayName', ''))
            row[2].text = str(c.get('Description', ''))
            row[3].text = str(c.get('ControlObjective', ''))
    else:
        doc.add_paragraph('No controls were found for the Performance Management domain.', style='Normal')

    # Additional policy items (generic)
    ...
    doc.add_paragraph('Performance Management Policy (v1.00) – Provides the baseline governance framework.', style='Normal')
